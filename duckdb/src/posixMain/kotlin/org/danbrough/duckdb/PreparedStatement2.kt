...
class PreparedStatement2(
	private val connection: Connection2,
	val sql: String,
	override val handle: duckdb_prepared_statementVar,
) : NativeObject<duckdb_prepared_statementVar> {
	...
	fun executeWithResult() = org.danbrough.duckdb.Result2(connection.memScope.alloc()).also {
		if (duckdb_execute_prepared(handle.value, it.handle.ptr) == DuckDBError)
			error("duckdb_execute_prepared failed: ${prepareError()}")
	}

	fun <R> executeWithResult(block: Result2.() -> R) =
